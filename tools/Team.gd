...
func get_next_member(member):
	...
	if index != -1:
		...
		if index == team_members.size() - 1:
			return team_members[0].get_ref()
		
		return team_members[index+1].get_ref()
	return null
...
func get_previous_member(member):
	...
	if index != -1:
		if team_members.size() == 1:
			return null
		if index == 0:
			return team_members[ team_members.size() - 1].get_ref()
		return team_members[index-1].get_ref()
	return null
...
func get_team_ai(only_ai := true) -> Array:
	var team_list := []
	#for character in get_tree().get_nodes_in_group("character"):
	for character_ref in team_members:
...
func _find_character(character) -> int:
	for index in range( team_members.size() ):
		if team_members[index].get_ref() == character:
			return index
	return -1
...
func _ready():
	...
	for character in get_tree().get_nodes_in_group("character"):
		if character.team == self.team:
			team_members.append( weakref(character) )
...
func _update_ai():
	...
	# Agressive
	#
	if team_members.size() >= max_team - 2 and not ennemies.empty():
		for team_member in team:
			if team_member.has_object():
...
func _on_constructor_build(name, object):
	...
	if name == "pill":
		...
		team_members.push_back( weakref(object) )
...
func _on_death(object):
	...
	var index = team_members.find_last(object)
	if index != -1:
		team_members.remove(index)
